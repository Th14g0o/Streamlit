...
from interfaces.admUI import AdmUI
...
class Menus:
    ...
    def MenuAdm():
        st.sidebar.header("Menu")
        op = st.sidebar.selectbox("Pagina atual:", ["Gerenciar Usuarios", "Gerenciar Jogos"])
        if op == "Gerenciar Usuarios":
            AdmUI.ManterUsuario()
        if op == "Gerenciar Jogos": 
            AdmUI.ManterJogos()
